...
class CIFailureCategory(str, Enum):
    """CI (test) failure category classification."""
    JEST_TEST_FAILURE = "jest_test_failure"
    CYPRESS_TEST_FAILURE = "cypress_test_failure"
...
class PullRequest(Base):
    ...
    workflow_runs = relationship("WorkflowRun", back_populates="pull_request", cascade="all, delete-orphan")
...
class WorkflowRun(Base):
    """Track GitHub Actions workflow runs associated with PRs."""
    ...
    run_id = Column(BigInteger, nullable=False, unique=True, index=True)  # workflow_run.id (64-bit)
    name = Column(String(255))
    event = Column(String(100))  # e.g. "pull_request"
    status = Column(String(50))  # e.g. "completed"
    conclusion = Column(String(50))  # e.g. "failure"
    ...
    analyses = relationship("WorkflowAnalysis", back_populates="workflow_run", cascade="all, delete-orphan")

    def __repr__(self) -> str:
        return f"<WorkflowRun {self.run_id} conclusion={self.conclusion}>"


class WorkflowAnalysis(Base):
    """Track CI failure analyses for GitHub Actions workflow runs."""
    ...
    # Analysis metadata
    status = Column(SQLEnum(AnalysisStatus), default=AnalysisStatus.PENDING, nullable=False, index=True)
    failure_category = Column(SQLEnum(CIFailureCategory), index=True)
    framework = Column(String(50))  # "jest", "cypress", "unknown"

    # Results
    summary = Column(Text)
    root_cause = Column(Text)  # Full markdown report
    ...
    # Relationships
    workflow_run = relationship("WorkflowRun", back_populates="analyses")

    def __repr__(self) -> str:
        return f"<WorkflowAnalysis {self.id} category={self.failure_category}>"
