...
from .comment_formatter import (
    ...
    format_ci_analysis_comment,
    format_ci_logs_comment,
    format_logs_comment,
...
async def health():
    ...
    return {
        ...
        "checks": {
            "database": "ok",
            "github": "ok" if settings.github_enabled else "unconfigured",
            "coolify": "ok" if settings.coolify_enabled else "unconfigured",
            "llm": "ok" if settings.llm_enabled else "unconfigured"
        }
    }
...
async def github_webhook(
    ...
    if event_type == "pull_request":
        ...
        background_tasks.add_task(handle_workflow_run_event, payload, db)
    else:
...
async def handle_issue_comment_event(payload: Dict, db: Session):
    ...
    if comment_lower in approval_keywords:
        ...
        # Trigger automatic fix (deployment or CI)
        await handle_apply_any_fix_command(installation_id, pr, comment_author, db)
        return
...
async def handle_workflow_run_event(payload: Dict, db: Session):
    ...
    if not (installation_id and repo_owner and repo_name and run_id):
        logger.warning("workflow_run_missing_required_fields", has_installation=bool(installation_id))
        return

    pull_requests = workflow_run.get("pull_requests", []) or []
    if not pull_requests:
        logger.info("workflow_run_has_no_pull_requests", run_id=run_id)
        return

    for pr_stub in pull_requests:
        ...
        if not pr:
            try:
                gh_pr = github_client.get_pull_request(installation_id, repo_owner, repo_name, pr_number)
                pr = PullRequest(
                    repo_owner=repo_owner,
                    repo_name=repo_name,
                    pr_number=pr_number,
                    branch_name=gh_pr.head.ref,
                ...
                logger.error("failed_to_fetch_pr_for_workflow_run", pr_number=pr_number, error=str(e))
                continue

        # Upsert workflow run
        wr = db.query(WorkflowRun).filter_by(run_id=run_id).first()
        if not wr:
            wr = WorkflowRun(
                pr_id=pr.id,
                run_id=run_id,
            ...
            wr.html_url = run_url
            wr.head_sha = head_sha or wr.head_sha
            wr.name = workflow_name or wr.name
            wr.run_attempt = run_attempt or wr.run_attempt
            wr.completed_at = datetime.utcnow()

        db.commit()

        # Download logs
        logs = github_client.download_workflow_run_logs(
            installation_id,
            repo_owner,
            repo_name,
            run_id,
        ...
        # Analyze CI failure (with repo access)
        branch = pr.branch_name
        analysis_result = await analyzer.analyze_ci(
            logs,
        ...
        framework = analyzer.detect_ci_framework(logs)
        category = {
            "jest": CIFailureCategory.JEST_TEST_FAILURE,
            "cypress": CIFailureCategory.CYPRESS_TEST_FAILURE,
            "unknown": CIFailureCategory.TEST_FAILURE,
        }.get(framework, CIFailureCategory.TEST_FAILURE)
        ...
        wa = WorkflowAnalysis(
            ...
            llm_prompt_tokens=analysis_result.prompt_tokens,
            llm_completion_tokens=analysis_result.completion_tokens,
            completed_at=datetime.utcnow(),
        )
        db.add(wa)
        db.commit()

        comment = format_ci_analysis_comment(
        ...
        github_client.post_or_update_comment(
            ...
            pr_number,
            comment,
            marker="<!-- incidentfox-ci-analysis -->",
        )


async def handle_apply_any_fix_command(
    installation_id: int,
    pr: PullRequest,
    author: str,
    db: Session,
):
    """Apply the best available fix (deployment failure preferred, otherwise CI failure)."""
    ...
    if deployment:
        await handle_apply_fix_command(installation_id, pr, author, db)
        return

    wr = db.query(WorkflowRun).filter_by(
        pr_id=pr.id,
        conclusion="failure",
    ).order_by(WorkflowRun.completed_at.desc()).first()

    if wr:
        await handle_apply_ci_fix_command(installation_id, pr, author, wr, db)
        return
    ...
    github_client.post_pr_comment(
        ...
        pr.repo_name,
        pr.pr_number,
        "❌ No failed deployment or failed CI run found for this PR.",
    )


async def handle_apply_ci_fix_command(
    installation_id: int,
    pr: PullRequest,
    author: str,
    ...
    """Apply a fix for the latest failed CI workflow run."""
    logger.info("handling_apply_ci_fix_command", pr_id=pr.id, author=author, run_id=workflow_run.run_id)

    analysis = db.query(WorkflowAnalysis).filter_by(
        workflow_run_id=workflow_run.id,
        status=AnalysisStatus.COMPLETED,
    ).order_by(WorkflowAnalysis.completed_at.desc()).first()
    ...
    if not analysis:
        github_client.post_pr_comment(
            ...
            pr.repo_name,
            pr.pr_number,
            "❌ No CI analysis found. Please wait for analysis to complete.",
        )
        return
    ...
    try:
        ...
        logs = workflow_run.full_logs or github_client.download_workflow_run_logs(
            installation_id, pr.repo_owner, pr.repo_name, workflow_run.run_id
        )

        # Get current PR head branch (authoritative)
        repo = github_client.get_repository(installation_id, pr.repo_owner, pr.repo_name)
        ...
        if not proposed_fix:
            github_client.post_pr_comment(
                ...
                pr.repo_name,
                pr.pr_number,
                "❌ Unable to generate an automatic fix for this CI failure. Manual intervention required.",
            )
            return
...
Confidence: {proposed_fix.confidence}"""
        ...
        success_message = f"""✅ **CI Fix Applied Successfully**
...
GitHub Actions should rerun checks automatically for the updated PR commit (or you can manually re-run the failed workflow).
...
*🦊 Powered by IncidentFox with Claude {settings.claude_model}*"""
        ...
        logger.error("ci_fix_generation_failed", error=str(e), pr_number=pr.pr_number)
        github_client.post_pr_comment(
            installation_id,
            pr.repo_owner,
            pr.repo_name,
            pr.pr_number,
            f"❌ Failed to generate CI fix: {str(e)}\n\nPlease review the error and try again.",
        )
        raise
...
async def handle_show_logs_command(
    ...
    # Prefer showing deployment logs if present
    deployment = db.query(Deployment).filter_by(
        pr_id=pr.id
    ).order_by(Deployment.started_at.desc()).first()

    if not deployment:
        # Fallback to CI logs (GitHub Actions workflow_run)
        wr = db.query(WorkflowRun).filter_by(
            pr_id=pr.id
        ).order_by(WorkflowRun.completed_at.desc()).first()
        ...
        if not wr:
            github_client.post_pr_comment(
                ...
                pr.repo_name,
                pr.pr_number,
                "❌ No deployment or CI run found for this PR."
            )
            return

        logs = wr.full_logs or github_client.download_workflow_run_logs(
            installation_id,
            pr.repo_owner,
            pr.repo_name,
            wr.run_id
        ...
        if not logs:
            github_client.post_pr_comment(
                ...
                pr.repo_name,
                pr.pr_number,
                "❌ No logs available for this CI run."
            )
            return

        comment = format_ci_logs_comment(logs, wr.run_id)
