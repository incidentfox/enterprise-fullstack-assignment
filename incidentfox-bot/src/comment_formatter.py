...
def format_ci_analysis_comment(
    ...
    """Format CI failure analysis as a PR comment."""
    lines = [
        "<!-- incidentfox-ci-analysis -->",
        "",
        ...
        "Other commands:",
        "",
        "- `/incidentfox show logs` - Show detailed logs (deployment or CI when available)",
        "",
    ]

    lines.extend([
        "<details>",
        "<summary>CI Run Details</summary>",
        "",
        f"- **Run ID:** `{run_id}`",
    ])
...
def format_ci_logs_comment(logs: str, run_id: int, max_lines: int = 120) -> str:
    """Format CI logs as a comment."""
    log_lines = logs.split("\n")
    total_lines = len(log_lines)
    display_lines = log_lines[-max_lines:] if total_lines > max_lines else log_lines
